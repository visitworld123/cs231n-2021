...
def affine_bn_relu_forward(x, w, b, gamma, beta, bn_param):
    """Convenience layer that performs a affine, a batch normalization, and a ReLU.
...
def affine_ln_relu_forward(x, w, b, gamma, beta, ln_param):
    """Convenience layer that performs a affine, a layer normalization, and a ReLU.
...
def affine_ln_relu_backward(dout, cache):
    ...
    """
    affine_cache, ln_cache, relu_cache = cache
    dan = relu_backward(dout, relu_cache)
    da, dgamma, dbeta = batchnorm_backward_alt(dan, ln_cache)
    dx, dw, db = affine_backward(da, affine_cache)
    return dx, dw, db, dgamma, dbeta

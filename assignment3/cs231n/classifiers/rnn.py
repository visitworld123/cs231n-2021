...
class CaptioningRNN:
    ...
    def __init__(
        ...
        self.params["W_embed"] /= 100 # V, W

        # Initialize CNN -> hidden state projection parameters
        self.params["W_proj"] = np.random.randn(input_dim, hidden_dim)
        self.params["W_proj"] /= np.sqrt(input_dim) # D, H
        self.params["b_proj"] = np.zeros(hidden_dim) # H, 

        # Initialize parameters for the RNN
        dim_mul = {"lstm": 4, "rnn": 1}[cell_type]
        self.params["Wx"] = np.random.randn(wordvec_dim, dim_mul * hidden_dim)
        self.params["Wx"] /= np.sqrt(wordvec_dim) # W, H for rnn
        self.params["Wh"] = np.random.randn(hidden_dim, dim_mul * hidden_dim)
        self.params["Wh"] /= np.sqrt(hidden_dim) # H, H for rnn
        self.params["b"] = np.zeros(dim_mul * hidden_dim) # H,  for rnn

        # Initialize output to vocab weights
        self.params["W_vocab"] = np.random.randn(hidden_dim, vocab_size)
        self.params["W_vocab"] /= np.sqrt(hidden_dim) # H, V
    ...
    def loss(self, features, captions):
        ...
        # backward
        dh, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(dx, cache3)
        
        if self.cell_type == 'rnn':
            dx, dh0, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(dh, cache2)
        elif self.cell_type == 'lstm':
            dx, dh0, grads['Wx'], grads['Wh'], grads['b'] = lstm_backward(dh, cache2)

...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):        
        curr_x = x[:, t, :].reshape(N, D)
        next_h, cache[t] = rnn_step_forward(curr_x, prev_h, Wx, Wh, b)
        h[:, t, :] = next_h
        prev_h = next_h
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    t = np.tanh(next_c)
    next_h = o * t
    cache = (x, prev_h, prev_c, Wx, Wh, i, f, o, g, t)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x, prev_h, prev_c, Wx, Wh, i, f, o, g, t = cache
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        curr_x = x[:, t, :].reshape(N, D)
        next_h, next_c, cache[t] = lstm_step_forward(curr_x, prev_h, prev_c, Wx, Wh, b)
        h[:, t, :] = next_h
        prev_h = next_h
        prev_c = next_c
...
def lstm_backward(dh, cache):
    ...
    for t in range(T - 1, -1, -1):
        # be careful about dh0 & dc
        dx[:, t, :], dh0, dc, _dWx, _dWh, _db = lstm_step_backward(dh[:, t, :] + dh0, dc, cache[t])
        dWx += _dWx
        dWh += _dWh
        db += _db

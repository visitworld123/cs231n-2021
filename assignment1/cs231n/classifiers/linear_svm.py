...
def svm_loss_vectorized(W, X, y, reg):
    ...
    scores = X.dot(W)  # (N, C)
    correct_scores = np.choose(y, scores.T)  # (N, ) 
    margins = scores - correct_scores.reshape(-1, 1) + 1  # (N, C) delta = 1

    loss_matrix = margins * (margins > 0)
    loss = np.sum(loss_matrix) - num_train # each row minis one, because j != y_i
    
    # another method to caculate loss

...
def svm_loss(x, y):
    ...
    loss = np.sum(loss_matrix) - num_train # -1 each row, because j != y_i
    loss /= num_train

    dx = np.zeros(x.shape)
    dx[margins > 0] = 1 # include j = y_i
    cnt = np.sum(dx, axis=1) # include j = y_i
    dx[np.arange(num_train), y] -= cnt # offset
    dx /= num_train
